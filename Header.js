import './Header.css';
import React from 'react';

function Header() {
    return (
      <div className="header">
        <div className='top'>
        <div className='nav'>
          <img className="imgnf" src="data:image/png;base64,"/>


<select id="Dropdown">
    <option value="option1" selected>English</option>
    <option value="option2">Telugu</option>
    <option value="option3">Hindi</option>
</select>

<button className='bt'>Sign in</button>
        </div>


        <div className='bdy'>
          <p className='mh'>Enjoy big movies, hit series and more from ₹ 149.</p>
          <p className='smh'>Join today. Cancel anytime.</p>
          <p className='smh'>Ready to watch? Enter your email to create or restart your membership.</p>
          <span><input id="ip" type='text' placeholder='Email address'></input><button id="btn">Get Started</button></span>
        </div>
        </div>
      <div className='bottom'>
      
      </div>
      </div>
    );
  }
  
  export default Header;